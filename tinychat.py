...
class TinychatRoom():
    def __init__(self, room, nick=None, username=None, passwd=None, roomPassword=None,
        ...
        self.color = TINYCHAT_COLORS["red"]
        self.topic = ""
        ...
        while self.connected and not self.reconnecting:
            ...
            try:
                ...
                if msg['msg'] == rtmp_protocol.DataTypes.COMMAND:
                    ...
                    if cmd == "nickinuse":
                        self._chatlog("The nickname "+self.nick+" is already being used. Adding _.", True)
                        self.setNick(self.nick+"_")
                        continue
                    ...
                    if cmd == "_result":
                        ...
                        # msg = {"msg": rtmp_protocol.DataTypes.COMMAND, 
                        #     "command": [u"" + "play", 0, None,] + [self.streams[-1].id]}
                        # self.connection.writer.write(msg)
                        # self.connection.writer.flush()
                        # self._chatlog("Starting stream #"+str(pars[0])+" for "+self.streams[-1].nick+".", True)
                        continue
...
# Actions.
    ...
    def _sendPublish(self):
        msg = {"msg": rtmp_protocol.DataTypes.COMMAND, "command": [u"" + "publish", 0, None, u"" + self.user.id, u"" + "live"]}
        debugPrint("CLIENT: " + str(msg), self.room)
...
# RTMP Connection Helper Functions.
    def __authHTTP(self):
        ...
        #                 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.100',
        ...
        self.headers = {'Host': 'tinychat.com',
        # 'Connection': 'keep-alive',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.100',
    ...
    def __getBauth(self):
        # Different token in a greenroom'd room.
        if self.greenroom:
            site = "greenroom"
        else:
            site = "tinychat"
        
        url = ("http://tinychat.com/api/broadcast.pw?nick="+self.nick+
            "&site="+site+"&name="+self.room+"&id="+self.user.id)
        r = self.s.request(method="GET", url=url, timeout=15)
    ...
    while True:
        ...
        try:
            ...
            if available and not pw and res == -1:
                ...
                if counter == 4:
                    room._chatlog("Bawt not in room! Reconnecting...", True)
                    room.reconnect()
            else:
                counter = 0
        except:
...
# Run standalone, without any bot extensions like xxx.
